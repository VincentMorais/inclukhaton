...
> {
  ...
  initState() {
    ...
    const selectedTheme = this.props.incluscore.themes.find(
      (theme) => theme.id === selectedThemeId
    );
    if (!selectedTheme) {
      return this.props.incluscoreAppGoTo(incluscoreThemesPath);
    }
    const userTheme = IncluscorePropertiesHelper.getUserThemeByIdThemeIdUser(
      this.state.launch,
      selectedTheme.id
    );
    ...
    let selectedQuestion = idQuestion
      ? selectedTheme.questions.find((q) => q.id === idQuestion)
      : null;
    if (!selectedQuestion) {
  ...
  userHaveFinishAllThemes() {
    for (const t of this.props.incluscore.themes) {
      const userTheme: UserThemeDto =
        IncluscorePropertiesHelper.getUserThemeByIdThemeIdUser(
          this.props.launch,
          t.id
        );
  ...
  propositionResult = () => {
    ...
    if (
      selectedQuestion.propositions.filter((p) => p.isAGoodAnswer).length > 1
    ) {
      ...
      <>
        ...
        <p className={"c-silver q-explanation text-left"}>
          {TextToInterpretedTextHelper.getInterpretation(
            tr(selectedQuestion, "answerExplanation")
          )}
        </p>
        <button
  ...
  public chooseAnswers = async (propositions: PropositionDto[]) => {
	...
	this.analyticsUtils.track(AnalyticsUtils.SCR_USER_ANSWER, {
			  ...
			  proposition: propositions?.map((p) => p.id),
	});	
	for (const proposition of propositions) {
		await IncluscoreAppQuestions.setChosenProposition(
			proposition,
			selectedTheme.id,
			selectedQuestion.id,
			userTheme?.id,
	...
	this.setState({
		chosenProposition: propositions[0],
	});
	this.UserAnswers = [];
}
  ...
  public static setChosenProposition = async (
    ...
    try {
      ...
      const savedUserTheme: UserThemeDto = await HttpRequester.postHttp(
        USER_THEME_SCR_CTRL,
        userThemeToSave
      );
      if (!savedUserTheme) {
      ...
      window.location.href =
        "incluscore-app/add-user-theme-error" + window.location.search;
      return null;
    }
  };
  ...
  findIndexOfCurrentQuestion() {
    ...
    return selectedTheme.questions
      .map((q) => q.id)
      .indexOf(selectedQuestion.id);
  }
  UserAnswers: PropositionDto[] = [];

  GetNbAnswer(proposition: PropositionDto) {
    console.log(this.UserAnswers);
    if (this.UserAnswers.length !== 0) {
      if (this.multipleAnswers) {
        this.chooseAnswer(this.UserAnswers[0]);
      } else {
        this.chooseAnswer(this.UserAnswers[0]);
      }
    }
  ...
  SelectProposition(proposition: PropositionDto) {
    const { selectedQuestion } = this.state;
    console.log(!!this.multipleAnswers(selectedQuestion));

    if (this.multipleAnswers(selectedQuestion)) {
      if (!this.UserAnswers.includes(proposition)) {
        this.UserAnswers.push(proposition);
        document.getElementById(proposition.id).setAttribute( "style", "background-color:var(--incluscore__strong-purple); color:white;"
          );
      } else {
        let index = this.UserAnswers.indexOf(proposition);
        if (index > -1) {
          this.UserAnswers.splice(index, 1);
        }
        document.getElementById(proposition.id).removeAttribute("style");
      }
    } else {
      if (this.UserAnswers.length > 0) {
        this.UserAnswers = [];
        document.querySelectorAll(".answer-btn").forEach((el) => el.removeAttribute("style"));
        this.UserAnswers.push(proposition);
        document.getElementById(proposition.id).setAttribute("style","background-color:var(--incluscore__strong-purple); color:white;"
          );
      } else {
        this.UserAnswers.push(proposition);
        document
          .getElementById(proposition.id)
          .setAttribute(
            "style",
            "background-color:var(--incluscore__strong-purple); color:white;"
          );
      }
    }
  }
  ...
  render() {
    ...
    return (
      <div className={"incluscore-app question"}>
        ...
        {chosenProposition ? (
          ...
          <>
            ...
            {this.multipleAnswers(selectedQuestion) && (
              <p className={"c-scr-grey"}>
                <Translation ns={["translation", "incluscore"]}>
                  {(t) => (
                    <>
                      {t("answers.multipleGoodAnswers", { ns: "incluscore" })}
                    </>
                  )}
                </Translation>
              </p>
            ...
            {selectedQuestion.propositions.map((proposition) => {
              return proposition.enabled ? (
                <button
                  ...
                  onClick={() => this.SelectProposition(proposition)}
                  key={proposition.id}
                  className={"answer-btn"}
                >
                  {TextToInterpretedTextHelper.getInterpretation(
            ...
            <button
              ...
              onClick={() => this.chooseAnswers(this.UserAnswers)}
            >
              Envoyer
            </button>
          </>
        ...
        />
        <div
          className="progress w-100 position-fixed"
        ...
        >
          <div
            ...
            style={{
              width: `${
                (this.findIndexOfCurrentQuestion() / nbQuestions) * 100
              }%`,
            }}
            aria-valuenow={
              (this.findIndexOfCurrentQuestion() / nbQuestions) * 100
